...
import com.jia.swipeback.demo.util.DisplayUtils;
import com.jia.swipeback.demo.util.Utils;
...
public class IWindowInsetHelper {
    private final int KEYBOARD_HEIGHT_BOUNDARY;
    private final WeakReference<IWindowInsetLayout> mWindowInsetLayoutWR;

    public IWindowInsetHelper(ViewGroup viewGroup, IWindowInsetLayout windowInsetLayout) {
        mWindowInsetLayoutWR = new WeakReference<>(windowInsetLayout);
        KEYBOARD_HEIGHT_BOUNDARY = Utils.dp2px(viewGroup.getContext(), 100);
    ...
    public boolean defaultApplySystemWindowInsets19(ViewGroup viewGroup, Rect insets) {
        ...
        if (insets.bottom >= KEYBOARD_HEIGHT_BOUNDARY) {
            DisplayUtils.setPaddingBottom(viewGroup, insets.bottom);
            insets.bottom = 0;
        } else {
            DisplayUtils.setPaddingBottom(viewGroup, 0);
        }
    ...
    public boolean defaultApplySystemWindowInsets21(ViewGroup viewGroup, WindowInsetsCompat insets) {
        ...
        if (insets.getSystemWindowInsetBottom() >= KEYBOARD_HEIGHT_BOUNDARY) {
            showKeyboard = true;
            DisplayUtils.setPaddingBottom(viewGroup, insets.getSystemWindowInsetBottom());
        } else {
            DisplayUtils.setPaddingBottom(viewGroup, 0);
        }
